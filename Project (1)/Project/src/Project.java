...
//                    }
    ...
    private static void addTokens(ArrayList<String> lines, ArrayList<String> tokens, ArrayList<String> errors, int[] rowColumn) {
        String str = "";
        int size;
        for (String currentLine : lines) {
            ...
            for (int j = 0; j < currentLine.length(); j++) {
                ...
                if (isBracket(j, currentLine)) {
                    ...
                    str = newStr.toUpperCase(Locale.ROOT) + String.format(" %d:%d", rowColumn[0], rowColumn[1]);
                    // add the current token to the list of tokens
                    tokens.add(str);
                    j += (size - 1);
                    rowColumn[1] += size;
                    ...
                    str = str.toUpperCase(Locale.ROOT) + String.format(" %d:%d", rowColumn[0], rowColumn[1]);
                    // add the current token to the list of tokens
                    tokens.add(str);
                    j += (size - 1);
                    rowColumn[1] += size;
